...
class SAGE(nn.Module):
    ...
    def inference(self, dataloader, feats):
        ...
        """
        device = feats.device
        y_hidden = torch.zeros(feats.shape[0], self.hidden_dim).to(device)
        for l, layer in enumerate(self.layers):
            y = torch.zeros(
                feats.shape[0],
            ...
            for input_nodes, output_nodes, blocks in dataloader:
                ...
                # # 保存教师模型倒数第二层的logits
                # if l == 0:
                #     y_hidden[output_nodes] = h
                # n = torch.sum(y_hidden, axis=1)
                # print("此时y_hidden中非零个数为：", torch.count_nonzero(n))
