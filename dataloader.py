...
def load_out_t_hidden(out_t_dir, dataset):
    it = np.load(out_t_dir.joinpath("out_hidden.npz"))
    return torch.from_numpy(np.load(out_t_dir.joinpath("out_hidden.npz"))["arr_0"])
...
class SparseGraph:
    ...
    def get_neighbors(self, idx):
        ...
        """
        return self.adj_matrix[idx].indices  # indices存储非零元素的列索引值
